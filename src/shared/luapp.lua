...
type str = string
type buf = buffer
local fmt, byte = string.format, string.byte
...
local MAX_HEX_STR = 1024
local HEX_OUT = buffer.create(4 * MAX_HEX_STR)
local HEX_SCRATCH = buffer.create(MAX_HEX_STR)

local function HEX_check_str(s: str): (buf, buf) -- out, scratch
    local strlen = #s
    if strlen > MAX_HEX_STR then
        return buffer.create(4 * strlen), buffer.create(strlen)
    end
    return HEX_OUT, HEX_SCRATCH
end
local DQ = byte('"')
local function _hexify(s: str, out: buf, scratch: buf, all: any?): str
    local strlen = #s
    assert(buffer.len(scratch) >= strlen)
    assert(buffer.len(out) >= 4 * strlen)
    buffer.writestring(scratch, 0, s)
    local wi = 0
    buffer.writeu8(out, wi, DQ)
    wi += 1
    if all then
        for i = 0, strlen - 1 do
            local c = buffer.readu8(scratch, i)
            buffer.writeu32(out, wi, buffer.readu32(BIN_TO_HEX, 4 * c))
            wi += 4
        end
        ...
        for i = 0, strlen - 1 do
            ...
            if esc ~= 0 then
                buffer.writeu16(out, wi, esc)
                wi += 2
            elseif 31 < c and c < 127 then
                buffer.writeu8(out, wi, c)
                wi += 1
            else
                buffer.writeu32(out, wi, buffer.readu32(BIN_TO_HEX, 4 * c))
                wi += 4
    ...
    buffer.writeu8(out, wi, DQ)
    wi += 1
    return buffer.readstring(out, 0, wi)
end
local function hexify(s: str)
    local out, scratch = HEX_check_str(s)
    return _hexify(s, out, scratch, false)
end
...
local RESPECT_TOSTRING_MT = false -- hacky for lua 5.2+
...
local function errfmt(argn,varargs,...)
  ...
  local function atom(x)
    if type(x) == 'string' then return format('%s:%s', hexify(x),'string')
...
type id_resolver = (number) -> str|number
local _id_resolver: id_resolver? = nil
...
local function lua2doc(obj,opt_root)
  if opt_root == nil or type(opt_root) == 'string' then
    local root_path = text(opt_root or '<#>')
    local key,value
    local tracker = {}
    ...
    function key(key)
      if type(key) == 'table' then
        if #key == 2 and ('<#null>' == key[2] or
           type(key[2]) == 'table' and #key[2] == 2)
        then
          return brkangles(text('pair: '..get_addr(key)))
        else return brkangles(text(key))
        end
      else
        if type(key) == 'number' then
          -- HACK: id hook
          if _id_resolver then
            key = _id_resolver(key)
          end
          return brackets(text(key))
        else
          if '<#null>' == key then return brackets(consnullsp)
          else
            if isLikeId(key) then return text(key),"ID"
            else
              if type(key) == 'string' then return brackets(str2doc(key))
              else
                if key == nil then return error'nil key?'
                else return brkangles(text(key))
                end
              end
            end
    ...
    function value(val,...)
      if type(val) == 'table' then
        if RESPECT_TOSTRING_MT and val.__tostring then return text(tostring(val)) end
        if tracker[val] then return tracker[val]
        else
          if #val == 2 and ('<#null>' == val[2] and not val[TAG] or
             type(val[2]) == 'table' and #val[2] == 2 and not val[TAG])
          then
            do
              tracker[val] = render_path{...}
              return listed(list2doc(val,{},...))
            end
          else
            do
              tracker[val] = render_path{...}
              return table2doc(val,...)
            end
          end
        end
      else
        if '<#null>' == val then return consnull
        else
          if type(val) == 'string' then return str2doc(val)
          else
            if type(val) == 'function' or type(val) == 'thread' or
               type(val) == 'userdata'
            then
              return angles(text(val))
            elseif type(val) == "number" then
              -- HACK: id hook
              if _id_resolver then
                val = _id_resolver(val)
              end
              return text(val)
            else
              return text(val)
    ...
    return value(obj,root_path)
  else error(errfmt(2,false,obj,opt_root))
  end
end

-- pp(any, [root-name]="<#>", [page-width]=160, [line-fill]=0.8) -> string
-- @root-name -- optional name for the topmost table if you expect it to be recursive
-- @page-width -- optional max page width
-- @line-fill -- max desired (line width)/(page width)
local function pp(obj, root, width, fill)
  if root == nil
    or type(root) == 'string' and width == nil
    or type(width) == 'number' and (fill == nil or type(fill) == 'number')
    then return layout(lua2doc(obj, root), width, fill)
  else
    error(errfmt(4, false, obj, root, width, fill))
  end
end
...
-------------------
    ...
    luapp.hex = hexify
    luapp.set_id_resolver = function(r: id_resolver?)
      _id_resolver = r
    end
end -- unformatted end
...
type luapp = {
  pp: typeof(luapp.pp),
  luapp: typeof(luapp.pp),
  zpp: typeof(luapp.zpp),
  errfmt: typeof(luapp.errfmt),
  hex: typeof(luapp.hex),
  set_id_resolver: typeof(luapp.set_id_resolver)
}
...
local t = {
    ...
    "\x01\x02\x00",
    { 33, "hello" },
    { 1, 2, 3, 4, 5, 6, 7, 8, 9, 0, { 31, 32, 33 } },
    { 3.141592653589793 },
    { { { { { { 101 } } } } } },
    { 0, 1, 3.1415927410125732 },
    Field2 = { 12, a = 97 },
}
...
assert(hexify(luapp.pp(t).. "\n") == hexify(s))

--[[ (!) can cause circular reference
local Id = require("Id")
luapp.set_id_resolver(Id.pp)
local DATA = {
  [Id.Pet.NONE] = Id.Egg.NONE
}
assert(luapp.pp(DATA) == "{[Id.Pet.NONE]=Id.Egg.NONE}")
--]]
warn("[luapp -- ok]")

return (luapp :: any) :: luapp
