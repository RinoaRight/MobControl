...
-- The Array structure imitates 0-based index arrays to help with the port from JavaScript to Lua
local Array = {}
Array.__index = Array
...
function MatFunctions:Test()
    ...
    terms:ForEach(function(i, v)
        print(self:NumberFormat(v, precision, width) .. " * x^" .. i)
    end)
    local cc = self:CorrelationCoefficient(data, terms)
    local se = self:StandardError(data, terms)
    print("cc = " .. self:NumberFormat(cc, precision, width))
    print("se = " .. self:NumberFormat(se, precision, width))
end
...
function MatFunctions:ProcessData(data, p)
    ...
    data:ForEachMut(function(i, v)
        return Pair.Coerce(v)
    end)
...
function MatFunctions:ProcessArray(ys: { number }, p: number?)
    p = p or 2
    p = math.clamp(p :: number, 0, 18)
    local prs = {}
    for i, v in ys do
        prs[i] = { i, v }
    end
    ...
    print("--")
    local data = Array.FromTable(prs)
    data:ForEachMut(function(i, v)
        return Pair.Coerce(v)
    end)
    local terms = self:ComputeCoefficients(data, p)
    ...
    local se = self:StandardError(data, terms)
    warn("cc: " .. cc, "se: " .. se)
    return table.unpack(terms._tbl)
end
...
function RbxPolyReg.ToFunctionSourceCode(terms, clampMin, clampMax)
	...
	local code = {"-- stylua: ignore\nlocal function _solve(x)\n  return math.clamp(\n"}
	local size = terms:Size()
	terms:ForEach(function(i, v)
		local line = ("%g * math.pow(x, %i)"):format(v, i)
		if (i == (size - 1)) then
			line = (line .. ",")
		else
			line = (line .. " +")
